  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 border-t border-wax-300/50">
        <Link
          href={`/hives/${hive.id}`}
          className="px-3 py-2.5 text-center text-xs font-semibold text-hive-700 transition-colors hover:bg-honey-50/80 hover:text-honey-800"
        >
          Details
        </Link>
        <Link
          href={`/inspect?hive=${hive.id}`}
          className="inline-flex items-center justify-center gap-1.5 border-l border-wax-300/50 bg-honey-500/10 px-3 py-2.5 text-center text-xs font-semibold text-honey-800 transition-colors hover:bg-honey-500/20"
        >
